...
abstract class Document extends Controls_Stack {
	...
	protected function register_document_controls() {
		...
		$is_published = isset($this->post) && $this->post ? (self::STATUS_PUBLISH === $this->post->post_status || self::STATUS_PRIVATE === $this->post->post_status) : false;

		if ( $is_published || $can_publish || ! Plugin::$instance->editor->is_edit_mode() ) {

			$statuses = $this->get_post_statuses();
			if ( isset($this->get_main_post()->post_status) && 'future' === $this->get_main_post()->post_status ) {
